...
int main(int argc,char **argv) {
 ...
 char lines[384][UPDATE_NOTE_LINE_BYTES];
 ...
 assert(Update_FormatNotes("## Changelog\nCorreção: atualização, coração e español.",lines,384)>=2);
 bool saw_utf8=false; for(unsigned i=0;i<384 && lines[i][0];i++) if(strstr(lines[i],"Correção")) saw_utf8=true;
 assert(saw_utf8);
 ...
 for(unsigned i=0;i<count;i++)assert(strlen(lines[i])<UPDATE_NOTE_LINE_BYTES);
